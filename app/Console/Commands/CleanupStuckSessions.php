...
class CleanupStuckSessions extends Command
{
    ...
    public function handle(ExamGradingService $grading): int
    {
        ...
        // Chunk agar tidak load ribuan sesi sekaligus ke memori (hindari OOM saat ujian serentak)
        ExamSession::query()
            ->where('status', ExamSession::STATUS_IN_PROGRESS)
            ->whereNotNull('started_at')
            ->with(['examSchedule.subject', 'examSchedule.examPeriod'])
            ->chunkById(100, function ($candidates) use ($grading, $deadlineGrace, &$cleaned) {
                foreach ($candidates as $session) {
                    ...
                    //    individual + grace period berakhir.
