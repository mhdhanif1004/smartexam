<x-layouts.guru_mapel title="Soal Saya">
    <div class="space-y-6" x-data="importState()">
        @php
            $importRouteTemplate = route('guru_mapel.questions.import-template', '__TYPE__');
            $importFailedTemplate = route('guru_mapel.questions.import-failed', '__FILE__');
        @endphp
        <script>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kelola soal yang Anda buat. Soal hanya dapat
                    diakses dan diedit oleh Anda, dan hanya untuk mapel yang Anda ampu.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <button type="button" x-data="{}" @click="$dispatch('open-modal', 'import-questions')"
                ...
                <a href="{{ route('guru_mapel.questions.create') }}"
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <form method="GET" action="{{ route('guru_mapel.questions.index') }}"
                class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="flex-1">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari pertanyaan..."
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200" />
                </div>
                <div class="sm:w-64">
                ...
                <button type="submit"
                    class="inline-flex items-center rounded-lg bg-gray-800 px-4 py-2 text-sm font-semibold text-white transition hover:bg-gray-700 dark:bg-gray-200 dark:text-gray-900 dark:hover:bg-white">Filter</button>
                <div class="relative" x-data="{ exportOpen: false }">
                    <button type="button" @click="exportOpen = !exportOpen" @click.outside="exportOpen = false"
                        class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-600 transition hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                        </svg>
                        Export
                        <svg class="h-4 w-4 transition" :class="exportOpen && 'rotate-180'" fill="none"
                            viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                        </svg>
                    </button>
                    <div x-show="exportOpen" x-transition x-cloak
                        class="absolute right-0 z-20 mt-2 w-40 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800">
                        <button type="submit" name="format" value="xlsx"
                            formaction="{{ route('guru_mapel.questions.export') }}" @click="exportOpen = false"
                            class="flex w-full items-center gap-2 px-4 py-2 text-left text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700">
                            Export .xlsx
                        </button>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-gray-900">
                        ...
                        <tr class="group transition hover:bg-gray-50 dark:hover:bg-gray-800/40">
                            <td class="px-4 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="max-w-md text-sm font-medium text-gray-900 dark:text-gray-100 line-clamp-2">
                                    {{ Str::limit(strip_tags($question->question_text), 80) }}</div>
                                <div class="mt-1 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                                    ...
                                    @if ($question->image_path)
                                        <span class="inline-flex items-center gap-1 text-gray-400 dark:text-gray-500">
                                            <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l4.784 4.787M4.5 19.5h15A2.25 2.25 0 0021.75 17.25V6.75A2.25 2.25 0 0019.5 4.5h-15A2.25 2.25 0 002.25 6.75v10.5A2.25 2.25 0 004.5 19.5z" />
                                            </svg>
                                            Gambar
                                        </span>
                                    @endif
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                                {{ \App\Models\Question::TYPES[$question->type] ?? $question->type }}</td>
                            <td class="px-6 py-4 text-sm">
                                @php
                                    $targetParts = \App\Models\Classroom::summarizeTargetParts($question->classrooms->pluck('id'));
                                @endphp
                                <div class="flex flex-wrap gap-1">
                                    @forelse ($targetParts as $part)
                                        <span
                                            class="inline-flex rounded-md bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300">
                                            @if ($part['type'] === 'grade')
                                ...
                                @php
                                    $guruWeightBadges = [];
                                @endphp
                                @foreach (($question->classrooms->pluck('id') ?? collect()) as $cid)
                                    @php
                                        $chk = $weightChecks[$question->subject_id][$cid] ?? null;
                                        if ($chk !== null) {
                                            $guruWeightBadges[] = ['cid' => $cid, 'name' => $classroomIdToName[$cid] ?? "Kelas #{$cid}", 'total' => (float) ($chk['total'] ?? 0), 'delta' => (float) ($chk['delta'] ?? 0), 'status' => $chk['status'] ?? 'ok'];
                                        }
                                    @endphp
                                @endforeach
                            ...
                            <td class="px-6 py-4 text-sm">
                                @if ($question->is_active)
                                    <span
                                        class="inline-flex rounded-md bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300">Aktif</span>
                                @else
                                    <span
                                        class="inline-flex rounded-md bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-600 dark:bg-gray-700/40 dark:text-gray-300">Nonaktif</span>
                                @endif
                            </td>
                            @php
                            ...
                            <td class="px-6 py-4 text-sm">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button type="button"
                                            @click="deleteUrl = @js(route('guru_mapel.questions.destroy', $question)); deleteDescription = @js($deleteConfirmText); $dispatch('open-modal', 'confirm-delete')"
                                            class="rounded-md bg-rose-50 px-3 py-2.5 text-xs font-semibold text-rose-700 transition hover:bg-rose-100 dark:bg-rose-500/10 dark:text-rose-300 dark:hover:bg-rose-500/20 sm:py-1.5">Hapus</button>
                                    </div>
                                </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                ...
                                <a href="{{ route('guru_mapel.questions.create') }}"
                                    class="mt-4 inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-700">
            ...
            <x-modal name="confirm-bulk-delete" maxWidth="sm" focusable>
                <div class="p-6">
                    <div class="flex items-start gap-4">
                        <div
                            class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-rose-100 dark:bg-rose-500/10">
                        ...
                        <div>
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Konfirmasi Hapus Massal
                            </h2>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Yakin ingin menghapus <span
                                    x-text="selected.length" class="font-bold"></span> soal yang dipilih? Soal yang
                                sudah pernah dijawab peserta akan dilewati. Tindakan ini tidak dapat dibatalkan.</p>
                        </div>
                    </div>
