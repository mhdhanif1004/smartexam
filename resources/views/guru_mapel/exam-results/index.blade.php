<x-layouts.guru_mapel title="Hasil Ujian CBT">
    <div class="space-y-6">
        <div>
            <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100">Hasil Ujian CBT</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Lihat hasil ujian berbasis komputer (CBT) untuk mapel dan kelas yang Anda ampu.</p>
        </div>

        @include('admin.partials.flash')

        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <form method="GET" action="{{ route('guru_mapel.exam-results.index') }}" class="grid gap-4 sm:grid-cols-3">
                <div>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-gray-900">
                            @forelse ($schedules as $index => $schedule)
                                <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                    ...
                                    <td class="px-6 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $schedule->examPeriod?->name ?? '-' }}</td>
                                    <td class="px-6 py-3 text-right">
                                        <a href="{{ route('guru_mapel.exam-results.schedule', $schedule->id) }}"
