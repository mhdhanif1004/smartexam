    ...
    >
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <button type="button" @click="importState.reset(); $dispatch('open-modal', 'import-supervisors')" class="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm transition hover:bg-gray-50">
        ...
        <div x-show="selected.length > 0" x-transition class="flex items-center justify-between gap-3 rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-3">
            <p class="text-sm font-medium text-indigo-800">
                <span x-text="selected.length" class="font-bold"></span> pengawas dipilih <span class="text-xs text-indigo-500">(dari semua halaman)</span>
            </p>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse ($supervisors as $index => $supervisor)
                            <tr class="transition hover:bg-gray-50">
                                <td class="px-4 py-3">
                                    <input type="checkbox" :checked="selected.includes({{ $supervisor->id }})" @change="toggleSelect({{ $supervisor->id }})" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-8 text-center text-sm text-gray-500">Tidak ada data pengawas.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="p-6">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">Yakin ingin menghapus <span x-text="selected.length" class="font-bold"></span> data pengawas beserta akunnya? Tindakan ini tidak dapat dibatalkan.</p>
                    </div>
                </div>
        ...
        <x-modal name="export-supervisors" maxWidth="sm">
            <div class="p-6">
                <h2 class="text-lg font-semibold text-gray-900">Ekspor Data Pengawas</h2>
                <p class="mt-1 text-sm text-gray-500">Unduh data pengawas ke file Excel atau CSV.</p>
        ...
        <x-modal name="import-supervisors" maxWidth="2xl">
            <div class="p-6">
                <h2 class="text-lg font-semibold text-gray-900">Impor Data Pengawas</h2>
                <p class="mt-1 text-sm text-gray-500">Upload file Excel/CSV berisi kolom Nama dan Email.</p>
                ...
                <template x-if="importState.step === 1">
                    <div class="mt-5">
                        ...
                        <div class="mt-3">
                            <a href="{{ route('admin.supervisors.import-template') }}" class="text-sm font-medium text-indigo-600 transition hover:text-indigo-500">Unduh Template Impor (.xlsx)</a>
                        </div>
                    </div>
                </template>
