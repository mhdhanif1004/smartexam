<x-layouts.admin title="Master Data Kelas">
    <div x-data="{ deleteUrl: '' }" class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100">Master Data Kelas</h2>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kelola daftar kelas yang dipakai saat import siswa.</p>
            </div>
        ...
        <form method="GET" action="{{ route('admin.classrooms.index') }}" class="flex flex-col gap-3 lg:flex-row lg:items-center">
            ...
            <div class="flex gap-2">
                ...
                @if (request('search'))
                    <a href="{{ route('admin.classrooms.index') }}" class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">Reset</a>
                @endif
            </div>
        </form>
        ...
        <x-table :headers="['No', 'Nama Kelas', 'Jumlah Siswa', 'Aksi']">
            @forelse ($classrooms as $index => $classroom)
                <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $classrooms->firstItem() + $index }}</td>
                    <td class="px-4 py-3 text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $classroom->name }}</td>
                    <td class="px-4 py-3 text-sm">
                        <span class="inline-flex items-center rounded-full {{ $classroom->students_count > 0 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700/60 dark:text-gray-400' }} px-2.5 py-0.5 text-xs font-medium">{{ $classroom->students_count }} siswa</span>
                    </td>
                    <td class="px-4 py-3 text-sm">
