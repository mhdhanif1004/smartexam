<x-layouts.admin title="Data Kepala Sekolah">
    <div
        x-data="{
            deleteUrl: '',
            selected: [],
            sessionKey: 'smartexam_selected_kepala_sekolahs',
            bulkDeleteSuccess: @js(str_contains((string) session('success'), 'data kepala sekolah berhasil dihapus.')),
            visibleIds: @js($kepalaSekolahs->map(fn ($ks) => $ks->id)->values()),
            init() {
            ...
            bulkDeleteUrl: @js(route('admin.kepala-sekolahs.bulk-delete')),
        }"
        class="space-y-6"
    >
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100">Data Kepala Sekolah</h2>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kelola akun kepala sekolah.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('admin.kepala-sekolahs.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-500">
        ...
        <form method="GET" action="{{ route('admin.kepala-sekolahs.index') }}" class="flex flex-col gap-3 lg:flex-row lg:items-center">
            <div class="flex-1">
            ...
            <div class="flex gap-2">
                ...
                @if (request('search'))
                    <a href="{{ route('admin.kepala-sekolahs.index') }}" class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">Reset</a>
                @endif
            </div>
        </form>
        ...
        <div x-show="selected.length > 0" x-cloak x-transition class="flex items-center justify-between gap-3 rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-3 dark:border-indigo-500/30 dark:bg-indigo-500/10">
            <p class="text-sm font-medium text-indigo-800 dark:text-indigo-200">
                <span x-text="selected.length" class="font-bold"></span> kepala sekolah dipilih <span class="text-xs text-indigo-500 dark:text-indigo-400">(dari semua halaman)</span>
            </p>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-gray-900">
                        @forelse ($kepalaSekolahs as $index => $kepalaSekolah)
                            <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                <td class="px-4 py-3">
                                    <input type="checkbox" :checked="selected.includes({{ $kepalaSekolah->id }})" @change="toggleSelect({{ $kepalaSekolah->id }})" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800" />
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $kepalaSekolahs->firstItem() + $index }}</td>
                                <td class="px-4 py-3 text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $kepalaSekolah->user?->name }}</td>
                                <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $kepalaSekolah->user?->email }}</td>
                                @include('admin.partials.password-cell', ['user' => $kepalaSekolah->user])
                                <td class="px-4 py-3 text-sm">
                                    <x-badge-status :status="$kepalaSekolah->user?->is_active ? 'aktif' : 'nonaktif'" />
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    <div class="flex items-center gap-2">
                                        @if (Route::has('admin.kepala-sekolahs.show'))
                                            <a href="{{ route('admin.kepala-sekolahs.show', $kepalaSekolah) }}" class="rounded-md bg-gray-50 px-3 py-1.5 text-xs font-semibold text-gray-700 transition hover:bg-gray-100 dark:bg-gray-700/60 dark:text-gray-300 dark:hover:bg-gray-600">Detail</a>
                                        @endif
                                        <a href="{{ route('admin.kepala-sekolahs.edit', $kepalaSekolah) }}" class="rounded-md bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700 transition hover:bg-indigo-100 dark:bg-indigo-500/10 dark:text-indigo-300 dark:hover:bg-indigo-500/20">Edit</a>
                                        <button type="button" @click="deleteUrl = '{{ route('admin.kepala-sekolahs.destroy', $kepalaSekolah) }}'; $dispatch('open-modal', 'confirm-delete')" class="rounded-md bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-700 transition hover:bg-rose-100 dark:bg-rose-500/10 dark:text-rose-300 dark:hover:bg-rose-500/20">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Tidak ada data kepala sekolah.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="p-6">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Yakin ingin menghapus <span x-text="selected.length" class="font-bold"></span> data kepala sekolah beserta akunnya? Tindakan ini tidak dapat dibatalkan.</p>
                    </div>
                </div>
