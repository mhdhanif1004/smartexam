    ...
    >
        ...
        <div x-show="selected.length > 0" x-transition class="flex items-center justify-between gap-3 rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-3">
            <p class="text-sm font-medium text-indigo-800">
                <span x-text="selected.length" class="font-bold"></span> mata pelajaran dipilih <span class="text-xs text-indigo-500">(dari semua halaman)</span>
            </p>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Kode</th>
                            <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Nama</th>
                            <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Durasi Default</th>
                            <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Jumlah Soal</th>
                            <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Jadwal Ujian</th>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse ($subjects as $index => $subject)
                            <tr class="transition hover:bg-gray-50">
                                <td class="px-4 py-3">
                                    <input type="checkbox" :checked="selected.includes({{ $subject->id }})" @change="toggleSelect({{ $subject->id }})" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                                </td>
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if ($subject->exam_schedules_count > 0)
                                        <span class="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-medium text-amber-700">{{ $subject->exam_schedules_count }} jadwal</span>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-8 text-center text-sm text-gray-500">Tidak ada data mata pelajaran.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="p-6">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">Anda akan menghapus <span x-text="selected.length" class="font-bold"></span> mata pelajaran. Tindakan ini tidak dapat dibatalkan.</p>

                        <div x-show="preview.linkedCount > 0" x-transition class="mt-3 flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
