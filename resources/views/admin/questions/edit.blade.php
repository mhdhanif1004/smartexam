        ...
        >
                    ...
                    <div class="mt-5 space-y-3">
                        @foreach ($letters as $letter)
                            <div class="flex items-center gap-3">
                                <input type="radio" name="single_answer" value="{{ $letter }}" @checked($singleAnswer === $letter) class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500">
            ...
            <template x-if="type === 'multiple_choice'">
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="mt-5 space-y-3">
                        @foreach ($letters as $letter)
                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="multiple_answer[]" value="{{ $letter }}" @checked(in_array($letter, $multipleAnswers)) class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-gray-100 text-sm font-bold text-gray-700">{{ $letter }}</span>
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center">
                        <label class="flex items-center gap-3 rounded-lg border border-gray-200 px-4 py-3">
                            <input type="radio" name="true_false_answer" value="1" @checked($trueFalseAnswer === '1') class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500">
                            <span class="text-sm font-medium text-gray-800">Benar</span>
                        </label>
                        <label class="flex items-center gap-3 rounded-lg border border-gray-200 px-4 py-3">
                            <input type="radio" name="true_false_answer" value="0" @checked($trueFalseAnswer === '0') class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500">
            ...
            <template x-if="type === 'essay'">
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="mt-5">
                        <textarea id="essay_answer" name="essay_answer" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Tulis kunci jawaban atau rubrik (opsional)...">{{ $essayAnswer }}</textarea>
                    </div>
