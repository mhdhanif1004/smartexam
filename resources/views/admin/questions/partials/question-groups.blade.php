{{--
    Partial hierarki Bank Soal di dalam satu mata pelajaran (level 2–5):
    Guru pemilik → Jenis Ujian → Kelas target → tabel Soal.
    Variabel: $groups (array dari groupQuestionsForHierarchy), $subject (Subject),
    $search (string), $weightChecks, $classroomIdToName.
    Soal tanpa owner dikelompokkan di bucket "Belum Ada Guru"; soal tanpa
    exam_type di bucket "Belum Ditentukan". Hanya organisasi tampilan.
--}}
@if (empty($groups))
...
@else
    @foreach ($groups as $guruBucket)
        @php
            $guruIdentifier = $subject->id . '-guru-' . ($guruBucket['guru_id'] ?? 'belum-ada-guru');
            $guruCount = collect($guruBucket['types'])->sum('count');
        @endphp
        <div class="border-t border-gray-100 dark:border-gray-800">
            <div
                role="button"
                tabindex="0"
                @click="toggleGroup('{{ $guruIdentifier }}')"
                @keydown.enter="toggleGroup('{{ $guruIdentifier }}')"
                @keydown.space.prevent="toggleGroup('{{ $guruIdentifier }}')"
            ...
            >
                <span class="flex min-w-0 items-center gap-2.5">
                    <svg class="h-3.5 w-3.5 shrink-0 text-gray-400 transition-transform duration-200 dark:text-gray-500"
                         :class="isGroupExpanded('{{ $guruIdentifier }}') ? 'rotate-180' : ''"
                         fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                    </svg>
                    <span class="text-xs font-semibold text-gray-700 dark:text-gray-300">
                        @if ($guruBucket['guru_id'] === null)
                            <span class="text-amber-600 dark:text-amber-400">Belum Ada Guru</span>
                        @else
                            Guru: {{ $guruBucket['guru_name'] }}
                        @endif
                    </span>
            ...
            <div x-show="isGroupExpanded('{{ $guruIdentifier }}')" x-transition x-cloak
                 class="border-t border-gray-100 dark:border-gray-800">
                @foreach ($guruBucket['types'] as $typeBucket)
                    @php
                        $typeIdentifier = $guruIdentifier . '-jenis-' . ($typeBucket['exam_type_id'] ?? 'belum-ditentukan');
                    @endphp
                    <div class="border-t border-gray-100 first:border-t-0 dark:border-gray-800">
                        <div
                            role="button"
                            tabindex="0"
                            @click="toggleGroup('{{ $typeIdentifier }}')"
                            @keydown.enter="toggleGroup('{{ $typeIdentifier }}')"
                            @keydown.space.prevent="toggleGroup('{{ $typeIdentifier }}')"
                            class="flex w-full items-center justify-between gap-3 bg-white/60 px-8 py-2 text-left transition hover:bg-gray-50 dark:bg-gray-900/30 dark:hover:bg-gray-800/30"
                        >
                            <span class="flex min-w-0 items-center gap-2.5">
                                <svg class="h-3.5 w-3.5 shrink-0 text-gray-400 transition-transform duration-200 dark:text-gray-500"
                                     :class="isGroupExpanded('{{ $typeIdentifier }}') ? 'rotate-180' : ''"
                                     fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                                </svg>
                                <span class="text-xs font-medium text-gray-600 dark:text-gray-400">
                                    @if ($typeBucket['exam_type_id'] === null)
                                        <span class="text-amber-600 dark:text-amber-400">Belum Ditentukan</span>
                                    @else
                                        Jenis: {{ $typeBucket['type_name'] }}
                                    @endif
                                </span>
                                <span class="shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-600 dark:bg-gray-700/60 dark:text-gray-300">
                                    {{ $typeBucket['count'] }} soal
                                </span>
                            </span>
                        </div>

                        {{-- Level kelas + tabel soal (logika lama level 2/3). --}}
                        @foreach ($typeBucket['classrooms'] as $groupKey => $group)
                            @php
                                $groupIdentifier = $typeIdentifier . '-kelas-' . $groupKey;
                            @endphp
                            <div x-show="isGroupExpanded('{{ $typeIdentifier }}')" x-transition x-cloak
                                 class="border-t border-gray-100 dark:border-gray-800">
                                <div
                                    role="button"
                                    tabindex="0"
                                    ...
                                    class="flex w-full items-center justify-between gap-3 bg-gray-50/60 px-10 py-2.5 text-left transition hover:bg-gray-100/60 dark:bg-gray-800/20 dark:hover:bg-gray-800/40"
                                >
                                    <span class="flex min-w-0 items-center gap-2.5">
                                        <svg class="h-3.5 w-3.5 shrink-0 text-gray-400 transition-transform duration-200 dark:text-gray-500"
