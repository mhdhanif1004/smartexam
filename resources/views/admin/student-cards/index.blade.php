<x-layouts.admin :title="$type === 'pengawas' ? 'Kartu Login Pengawas' : 'Kartu Login Peserta'">
    <div class="space-y-6">
        ...
        @if ($type === 'pengawas')
            ...
            <div x-data="{ open: false, search: '', selectAll: false }" class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <button type="button" @click="open = !open" class="flex w-full items-center justify-between gap-3 px-5 py-4 text-left transition hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    <div class="flex items-center gap-3">
                        <svg class="h-5 w-5 shrink-0 text-gray-400 transition-transform" :class="open ? 'rotate-90' : ''" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
                        </svg>
                        <span class="text-sm font-bold text-gray-900 dark:text-gray-100">Pilih Pengawas ({{ $supervisors->count() }} orang)</span>
                    </div>
                    <span class="text-xs text-gray-500 dark:text-gray-400" x-text="open ? 'Klik untuk tutup' : 'Klik untuk buka'"></span>
                </button>
                ...
                <div x-show="open" x-transition x-cloak>
                    <div class="border-t border-gray-200 px-5 py-3 dark:border-gray-800">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div class="relative flex-1">
                                <svg class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
                                </svg>
                                <input type="text" x-model="search" placeholder="Ketik nama pengawas untuk mencari..."
                                    class="block w-full rounded-lg border-gray-300 pl-9 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200">
                            </div>
                            <div class="flex flex-wrap items-center gap-3">
                                <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                    <input type="checkbox" x-model="selectAll" @change="document.querySelectorAll('#card-form [data-supervisor-id]:visible').forEach(cb => cb.checked = $event.target.checked)" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800">
                                    Pilih Semua
                                </label>
                    ...
                    <form id="card-form" method="POST" x-on:submit="$refs.supervisorIds.value = [...document.querySelectorAll('#card-form [data-supervisor-id]:checked')].map(cb => cb.value).join(',')" class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        @forelse ($supervisors as $supervisor)
                            <label x-show="search === '' || '{{ strtolower($supervisor->user?->name ?? '') }}'.includes(search.toLowerCase())" class="flex cursor-pointer items-center gap-4 px-4 py-3 transition hover:bg-gray-50 dark:hover:bg-gray-800/50">
