...
@if ($type === 'pengawas')
            ...
            @if ($supervisors->isEmpty())
                <div class="rounded-xl border border-gray-200 bg-white p-8 text-center shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <p class="text-sm text-gray-500 dark:text-gray-400">Tidak ada pengawas{{ $selectedRoom ? ' di ruangan ini' : '' }}.</p>
                </div>
            @else
                <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="flex flex-col gap-3 border-b border-gray-200 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800">
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            Menampilkan <strong>{{ $supervisors->total() }}</strong> pengawas{{ $selectedRoom ? ' di ruangan ini' : '' }} (halaman {{ $supervisors->currentPage() }} dari {{ $supervisors->lastPage() }}). Centang pengawas tertentu, atau biarkan kosong untuk mencetak semua yang tampil.
                        </p>
                        <div class="flex flex-wrap items-center gap-3">
                            <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                <input type="checkbox" id="select-all-supervisors" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800">
                                Pilih Semua
                            </label>
                            <span class="text-sm text-gray-500 dark:text-gray-400">
                                <span id="selected-supervisor-count">0</span> dipilih
                            </span>
                            <button type="button" id="preview-supervisors-btn" class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">
                                Pratinjau
                            </button>
                            <button type="button" id="print-supervisors-btn" class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-500">
                    ...
                    <div class="divide-y divide-gray-100 dark:divide-gray-800">
                        @forelse ($supervisors as $supervisor)
                            <label class="flex cursor-pointer items-center gap-4 px-4 py-3 transition hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                <input type="checkbox" value="{{ $supervisor->id }}" data-supervisor-id="{{ $supervisor->id }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800">
                                ...
                                @if ($selectedRoom && $supervisor->rooms?->isNotEmpty())
                                    <span class="rounded-md bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">{{ $supervisor->rooms->first()->display_name }}</span>
                                @elseif ($supervisor->rooms?->isNotEmpty())
                                    <span class="rounded-md bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-600 dark:bg-gray-700/60 dark:text-gray-400">{{ $supervisor->rooms->first()->display_name }}</span>
                                @endif
            ...
            <form method="GET" action="{{ route('admin.student-cards.index') }}" class="flex flex-col gap-3 lg:flex-row lg:items-end rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                ...
                <div class="flex-1">
                    ...
                    <select name="class" id="class" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200">
                        <option value="" @selected($selectedClass === '')>Pilih Kelas</option>
    ...
    <script>
        (function() {
            const STORAGE_KEY = 'smartexam_selected_supervisors';
            ...
            function updateCounter() {
                var el = document.getElementById('selected-supervisor-count');
                if (el) el.textContent = getSelected().length;
            }

            function syncSelectAllCheckbox() {
                var selectAllCb = document.getElementById('select-all-supervisors');
                if (!selectAllCb) return;
                var cbs = document.querySelectorAll('[data-supervisor-id]');
                selectAllCb.checked = cbs.length > 0 && Array.from(cbs).every(function(cb) { return cb.checked; });
            }

            function restore() {
                var selected = getSelected();
                document.querySelectorAll('[data-supervisor-id]').forEach(function(cb) {
                    cb.checked = selected.indexOf(Number(cb.value)) !== -1;
                });
                updateCounter();
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Print button — collect ALL selected IDs from sessionStorage
                var printBtn = document.getElementById('print-supervisors-btn');
                if (printBtn) {
                    printBtn.addEventListener('click', function() {
                        var selected = getSelected();
                        if (selected.length === 0) {
                            alert('Tidak ada pengawas yang dipilih.');
                            return;
                        }
                        var form = document.createElement('form');
                        form.method = 'POST';
                        form.action = '{{ route('admin.student-cards.print') }}';
                        form.innerHTML = '<input type="hidden" name="_token" value="{{ csrf_token() }}">'
                            + '<input type="hidden" name="type" value="pengawas">'
                            + '<input type="hidden" name="supervisor_ids" value="' + selected.join(',') + '">';
                        var roomSelect = document.getElementById('room');
                        if (roomSelect && roomSelect.value) {
                            form.innerHTML += '<input type="hidden" name="room" value="' + roomSelect.value + '">';
                        }
                ...
                // Preview button — open preview in new tab
                var previewBtn = document.getElementById('preview-supervisors-btn');
                if (previewBtn) {
                    previewBtn.addEventListener('click', function() {
                        ...
                        var form = document.createElement('form');
                        form.method = 'POST';
                        form.action = '{{ route('admin.student-cards.preview') }}';
                        form.target = '_blank';
                        form.innerHTML = '<input type="hidden" name="_token" value="{{ csrf_token() }}">'
                            + '<input type="hidden" name="type" value="pengawas">'
                            + '<input type="hidden" name="supervisor_ids" value="' + selected.join(',') + '">';
