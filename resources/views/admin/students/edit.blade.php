<x-layouts.admin title="Edit Siswa">
    <div class="space-y-6">
        <div>
            <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100">Edit Siswa</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui data siswa dan akun penggunanya.</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.students.update', $student) }}" class="max-w-2xl space-y-6">
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                ...
                <label class="mt-5 flex w-fit items-center gap-2">
                    <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $student->user?->is_active)) class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800">
