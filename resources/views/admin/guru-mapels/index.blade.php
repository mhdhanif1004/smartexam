<x-layouts.admin title="Data Guru Mapel">
    <div
        x-data="{
            deleteUrl: '',
            selected: [],
            sessionKey: 'smartexam_selected_guru_mapels',
            bulkDeleteSuccess: @js(str_contains((string) session('success'), 'data guru mapel berhasil dihapus.')),
            visibleIds: @js($guruMapels->map(fn ($guru) => $guru->id)->values()),
            init() {
            ...
            bulkDeleteUrl: @js(route('admin.guru-mapels.bulk-delete')),
            importState: {
                step: 1,
                file: null,
                busy: false,
                ...
                failedUrl() {
                    return @js(route('admin.guru-mapels.import-failed', '__FILE__'))
                ...
                validate() {
                    ...
                    fetch(@js(route('admin.guru-mapels.import-validate')), {
                        method: 'POST',
                        body: formData,
                ...
                confirm() {
                    this.busy = true;
                    this.message = '';
                    fetch(@js(route('admin.guru-mapels.import-confirm')), {
                        method: 'POST',
                        headers: { 'X-CSRF-TOKEN': @js(csrf_token()), 'Accept': 'application/json' },
                    })
                        .then((response) => response.json().then((data) => ({ ok: response.ok, data })))
    ...
    >
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kelola akun guru mapel beserta penugasan mapel-kelasnya.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <button type="button" @click="importState.reset(); $dispatch('open-modal', 'import-guru-mapels')" class="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm transition hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                    </svg>
                    Impor
                </button>
                <a href="{{ route('admin.guru-mapels.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-500">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <form method="GET" action="{{ route('admin.guru-mapels.index') }}" class="flex flex-col gap-3 lg:flex-row lg:items-center">
            <div class="flex-1">
            ...
            <div class="flex gap-2">
                ...
                @if (request('search'))
                    <a href="{{ route('admin.guru-mapels.index') }}" class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">Reset</a>
                @endif
            </div>
        </form>
        ...
        <div x-show="selected.length > 0" x-transition class="flex items-center justify-between gap-3 rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-3 dark:border-indigo-500/30 dark:bg-indigo-500/10">
            <p class="text-sm font-medium text-indigo-800 dark:text-indigo-200">
                <span x-text="selected.length" class="font-bold"></span> guru mapel dipilih <span class="text-xs text-indigo-500 dark:text-indigo-400">(dari semua halaman)</span>
            </p>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-gray-900">
                        @forelse ($guruMapels as $index => $guru)
                            <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                <td class="px-4 py-3">
                                    <input type="checkbox" :checked="selected.includes({{ $guru->id }})" @change="toggleSelect({{ $guru->id }})" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800" />
                                </td>
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('admin.guru-mapels.show', $guru) }}" class="rounded-md bg-gray-50 px-3 py-1.5 text-xs font-semibold text-gray-700 transition hover:bg-gray-100 dark:bg-gray-700/60 dark:text-gray-300 dark:hover:bg-gray-600">Detail</a>
                                        <a href="{{ route('admin.guru-mapels.edit', $guru) }}" class="rounded-md bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700 transition hover:bg-indigo-100 dark:bg-indigo-500/10 dark:text-indigo-300 dark:hover:bg-indigo-500/20">Edit</a>
                                        <button type="button" @click="deleteUrl = '{{ route('admin.guru-mapels.destroy', $guru) }}'; $dispatch('open-modal', 'confirm-delete')" class="rounded-md bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-700 transition hover:bg-rose-100 dark:bg-rose-500/10 dark:text-rose-300 dark:hover:bg-rose-500/20">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
            ...
            <div class="p-6">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Yakin ingin menghapus <span x-text="selected.length" class="font-bold"></span> data guru mapel beserta akunnya? Tindakan ini tidak dapat dibatalkan.</p>
                    </div>
                </div>
        ...
        <x-modal name="import-guru-mapels" maxWidth="2xl">
            <div class="p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Impor Guru Mapel</h2>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Upload file Excel/CSV berisi kolom Nama, Mapel, Tingkat, dan Kelas.</p>
                ...
                <template x-if="importState.step === 1">
                    <div class="mt-5">
                        ...
                        <div class="mt-3">
                            <a href="{{ route('admin.guru-mapels.import-template') }}" class="text-sm font-medium text-indigo-600 transition hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300">Unduh Template Impor (.xlsx)</a>
                        </div>
                    </div>
                </template>
                    ...
                    <div class="mt-5">
                        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                            ...
                            <div class="rounded-lg bg-indigo-50 p-4 text-center dark:bg-indigo-500/10">
                                ...
                                <p class="text-xs font-medium text-indigo-600 dark:text-indigo-400">Guru Baru</p>
                            </div>
                            <div class="rounded-lg bg-amber-50 p-4 text-center dark:bg-amber-500/10">
                                <p class="text-2xl font-bold text-amber-700 dark:text-amber-300" x-text="importState.result?.assignments ?? 0"></p>
                                <p class="text-xs font-medium text-amber-600 dark:text-amber-400">Penugasan</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-4 rounded-lg border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-800 dark:bg-emerald-500/10">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-sm text-emerald-700 dark:text-emerald-400">
                                    <span class="font-bold" x-text="importState.finished?.created ?? 0"></span> guru mapel baru ditambahkan,
                                    <span class="font-bold" x-text="importState.finished?.updated ?? 0"></span> guru diperbarui,
                                    <span class="font-bold" x-text="importState.finished?.assignments ?? 0"></span> penugasan tersimpan.
                                </p>
