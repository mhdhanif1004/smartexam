<x-layouts.admin title="Edit Guru Mapel">
    <div class="space-y-6">
        <div>
            <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100">Edit Guru Mapel</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui data akun guru mapel. Penugasan mapel-kelas dikelola lewat halaman Detail.</p>
        </div>

        <form method="POST" action="{{ route('admin.guru-mapels.update', $guruMapel) }}" class="max-w-2xl space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                ...
                <div class="mt-5 grid gap-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" value="{{ old('name', $guruMapel->user?->name) }}" required autofocus />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="email" :value="__('Email')" />
                        <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" value="{{ old('email', $guruMapel->user?->email) }}" required />
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="nip" :value="__('NIP')" />
                        <x-text-input id="nip" name="nip" type="text" class="mt-1 block w-full" value="{{ old('nip', $guruMapel->nip) }}" placeholder="Opsional" />
                ...
                <label class="mt-5 flex w-fit items-center gap-2">
                    <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $guruMapel->user?->is_active)) class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800">
