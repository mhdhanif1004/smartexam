<x-layouts.admin title="Ruangan Ujian">
    <div
        x-data="{
            ...selectionManager({
                sessionKey: 'smartexam_selected_rooms',
                visibleIds: @js($rooms->map(fn ($room) => $room->id)->values()),
                bulkDeleteSuccess: @js(str_contains((string) session('success'), 'ruangan berhasil dihapus.')),
                bulkDeleteUrl: @js(route('admin.rooms.bulk-delete')),
            }),
            deleteUrl: '',
        }"
        class="space-y-6"
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead class="bg-gray-50 dark:bg-gray-800">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">No</th>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-gray-900">
                        @forelse ($rooms as $index => $room)
                            <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                ...
                                <td class="px-4 py-3.5 text-sm text-gray-500 dark:text-gray-400">{{ $rooms->firstItem() + $index }}</td>
                                <td class="px-4 py-3.5 text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $room->display_name }}</td>
                                <td class="px-4 py-3.5 text-sm text-gray-700 dark:text-gray-300">{{ $room->capacity }} peserta</td>
            ...
            <div class="p-6">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Yakin ingin menghapus <span x-text="selected.length" class="font-bold"></span> ruangan yang dipilih? Tindakan ini tidak dapat dibatalkan.</p>
                    </div>
                </div>
