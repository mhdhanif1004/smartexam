...
<form id="room-form" method="POST" action="{{ $action }}" class="space-y-6">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <h3 class="text-base font-bold text-gray-900">Informasi Ruangan</h3>
        <div class="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <div>
                ...
                <x-text-input id="capacity" name="capacity" type="number" min="0" readonly class="mt-1 block w-full bg-gray-100 text-gray-500" value="{{ old('capacity', count($assignedSet)) }}" />
                <p class="mt-1 text-xs text-gray-400">Terisi otomatis sesuai jumlah siswa yang dicentang di bawah.</p>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <h3 class="text-base font-bold text-gray-900">Pengawas Ruangan</h3>
        <p class="mt-1 text-sm text-gray-500">Satu pengawas untuk satu ruangan. Pengawas yang sedang bertugas di ruangan lain akan dipindahkan ke ruangan ini.</p>

        <div class="mt-4 max-w-xl">
            <label for="supervisor_id" class="mb-1 block text-sm font-medium text-gray-700">Pilih Pengawas</label>
            <select name="supervisor_id" id="supervisor_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        ...
        <div id="supervisor-warning" data-current-room="{{ $currentRoomName }}" class="mt-3 hidden rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <h3 class="text-base font-bold text-gray-900">Penempatan Siswa</h3>
            ...
            <div class="shrink-0 rounded-lg bg-indigo-50 px-4 py-2 text-sm font-bold text-indigo-700">
        ...
        <div class="mt-4 flex flex-col gap-3 sm:flex-row">
            <div class="sm:w-56">
                <label for="class-filter" class="mb-1 block text-xs font-medium text-gray-600">Filter Kelas</label>
                <select id="class-filter" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div class="flex-1">
                <label for="student-search" class="mb-1 block text-xs font-medium text-gray-600">Cari Nama / NISN</label>
                <input type="search" id="student-search" placeholder="contoh: Budi atau 1234567890..." class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            </div>
        </div>

        @if ($allStudents->isEmpty())
            <p class="mt-4 rounded-lg bg-gray-50 px-4 py-6 text-center text-sm text-gray-500">
        ...
        @else
            <div id="student-checklist" class="mt-4 grid grid-cols-1 gap-1.5 md:grid-cols-2 lg:grid-cols-3">
                @foreach ($allStudents as $student)
                    <label class="student-item flex cursor-pointer items-start gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm transition hover:border-indigo-300 hover:bg-indigo-50/40 {{ isset($assignedSet[$student->id]) ? 'border-indigo-200 bg-indigo-50/60' : '' }}"
                           data-class="{{ $student->class_name }}"
                           data-name="{{ mb_strtolower($student->user?->name ?? '') }}"
                           data-nisn="{{ $student->nisn }}">
                        <input type="checkbox" name="student_ids[]" value="{{ $student->id }}" {{ isset($assignedSet[$student->id]) ? 'checked' : '' }} class="mt-0.5 h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        <span class="min-w-0">
                            <span class="block truncate font-medium text-gray-900">{{ $student->user?->name }}</span>
                            <span class="block text-xs text-gray-500">{{ $student->nisn }} &middot; {{ $student->class_name }}</span>
                        </span>
                    </label>
                @endforeach
            </div>
        ...
        <div class="mt-5 flex flex-col gap-3 border-t border-gray-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
            <p class="text-sm text-gray-500">Siswa yang tidak dicentang lagi akan dilepas dari ruangan ini.</p>
            <div class="flex justify-end gap-3">
                <a href="{{ route('admin.rooms.index') }}" class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50">Batal</a>
