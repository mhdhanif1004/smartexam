        ...
        @else
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div class="flex items-center gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400">
                ...
                <div class="flex items-center gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-rose-100 text-rose-600 dark:bg-rose-500/10 dark:text-rose-400">
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
                        </svg>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Peserta Tidak Hadir</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-gray-100"><span class="summary-value" x-text="summaries.totals.absent">{{ $totals['absent'] }}</span></p>
                    </div>
                </div>
                <div class="flex items-center gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400">
                ...
                <div class="flex items-center gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-rose-100 text-rose-600 dark:bg-rose-500/10 dark:text-rose-400">
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M22 10.5h-6m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 0112 21.75c-2.973 0-5.703-.92-8-2.515zM18.75 3v4.5m0 0h4.5m-4.5 0H14.25m4.5 0l-4.5 4.5" />
                        </svg>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Pengawas Tidak Hadir</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-gray-100"><span class="summary-value" x-text="summaries.totals.supervisorAbsent">{{ $totals['supervisorAbsent'] }}</span></p>
                    </div>
                </div>
            </div>
                ...
                <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <button type="button" @click="togglePeriod({{ $periodIndex }})" class="flex w-full flex-col gap-3 px-5 py-4 text-left sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
                            <span class="text-emerald-600 dark:text-emerald-400">Hadir <strong class="summary-value" x-text="periodSummary({{ $periodIndex }}).present">{{ $periodPresent }}</strong></span>
                            <span class="text-rose-600 dark:text-rose-400">Tidak Hadir <strong class="summary-value" x-text="periodSummary({{ $periodIndex }}).absent">{{ $periodAbsent }}</strong></span>
                            <span class="text-gray-300 dark:text-gray-600">&middot;</span>
                            <span class="text-gray-500 dark:text-gray-400">Pengawas <strong class="text-emerald-600 dark:text-emerald-400"><span class="summary-value" x-text="periodSummary({{ $periodIndex }}).supervisorPresent">{{ $periodSupervisorPresent }}</span></strong>/<strong class="text-rose-600 dark:text-rose-400"><span class="summary-value" x-text="periodSummary({{ $periodIndex }}).supervisorAbsent">{{ $periodSupervisorAbsent }}</span></strong></span>
                                    ...
                                    <button type="button" @click="toggleRoom(@js($roomKey))" class="w-full px-4 py-3 text-left">
                                        ...
                                        <div class="mt-3 grid grid-cols-2 gap-3">
                                            ...
                                            <div class="rounded-lg bg-rose-50 px-3 py-2 dark:bg-rose-500/10">
                                                ...
                                                <p class="text-xl font-bold text-rose-700 dark:text-rose-300"><span class="summary-value" x-text="roomSummary(@js($periodIndex), @js($roomIndex)).absent">{{ $room['absent'] }}</span> <span class="text-sm font-medium text-rose-500 dark:text-rose-400">/ <span class="summary-value" x-text="roomSummary(@js($periodIndex), @js($roomIndex)).total">{{ $roomTotal }}</span></span></p>
                                            </div>
                                        </div>
                                        <div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                            Pengawas: <strong class="text-emerald-600 dark:text-emerald-400"><span class="summary-value" x-text="roomSummary(@js($periodIndex), @js($roomIndex)).supervisorPresent">{{ $room['supervisorPresent'] }}</span></strong> hadir &middot; <strong class="text-rose-600 dark:text-rose-400"><span class="summary-value" x-text="roomSummary(@js($periodIndex), @js($roomIndex)).supervisorAbsent">{{ $room['supervisorAbsent'] }}</span></strong> tidak hadir
                                            <span x-show="roomSummary(@js($periodIndex), @js($roomIndex)).supervisorUnchecked > 0" x-cloak>&middot; <span class="text-gray-400 dark:text-gray-500"><span class="summary-value" x-text="roomSummary(@js($periodIndex), @js($roomIndex)).supervisorUnchecked">{{ $supervisorUnchecked }}</span> belum dicek</span></span>
                                        </div>
                                        <p x-show="roomSummary(@js($periodIndex), @js($roomIndex)).unchecked > 0" x-cloak class="mt-2 text-xs font-medium text-amber-600 dark:text-amber-400"><span class="summary-value" x-text="roomSummary(@js($periodIndex), @js($roomIndex)).unchecked">{{ $room['unchecked'] }}</span> peserta belum dicek pengawas.</p>
