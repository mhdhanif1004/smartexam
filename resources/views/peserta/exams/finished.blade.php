<x-layouts.peserta :title="'Selesai - '.($session->examSchedule->subject?->name ?? 'Ujian')">
    <div class="mx-auto max-w-lg space-y-6">
        <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-8 text-center shadow-sm dark:border-emerald-800 dark:bg-emerald-500/10">
            ...
            <h2 class="mt-4 text-2xl font-bold text-gray-900 dark:text-gray-100">Ujian Berhasil Dikumpulkan</h2>
            <p class="mt-1 text-sm text-emerald-700 dark:text-emerald-300">Jawabanmu sudah diterima. Jangan lupa berdoa untuk hasil terbaik!</p>
        </div>

        @include('admin.partials.flash')

        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <h3 class="border-b border-gray-100 pb-3 text-base font-bold text-gray-900 dark:border-gray-800 dark:text-gray-100">Ringkasan Ujian</h3>
            <dl class="mt-4 space-y-3 text-sm">
                <div class="flex justify-between gap-4">
                    <dt class="text-gray-500 dark:text-gray-400">Mata Pelajaran</dt>
                    <dd class="font-semibold text-gray-900 dark:text-gray-100">{{ $session->examSchedule->subject?->name }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                ...
                <div class="flex justify-between gap-4">
                    ...
                    <dd class="font-semibold text-gray-900 dark:text-gray-100">{{ \Illuminate\Support\Carbon::now()->startOfDay()->addSeconds($workingSeconds)->format('H:i:s') }}</dd>
                </div>
                <div class="flex justify-between gap-4">
