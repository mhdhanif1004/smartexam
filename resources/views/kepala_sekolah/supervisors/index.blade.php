<x-layouts.kepala_sekolah title="Data Pengawas">
    <div class="space-y-6">
        <div>
            <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100">Data Pengawas</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Daftar pengawas terdaftar — hanya untuk dilihat.</p>
        </div>

        @include('admin.partials.flash')

        <form method="GET" action="{{ route('kepala_sekolah.supervisors.index') }}" class="flex flex-col gap-3 lg:flex-row lg:items-center">
            <div class="flex-1">
            ...
            <div class="flex gap-2">
                ...
                @if (request('search') || request('room'))
                    <a href="{{ route('kepala_sekolah.supervisors.index') }}" class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">Reset</a>
                @endif
            </div>
        </form>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-gray-900">
                        @forelse ($supervisors as $index => $supervisor)
                            <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if ($supervisor->room)
                                        <span class="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">{{ $supervisor->room->display_name }}</span>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Tidak ada data pengawas.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
