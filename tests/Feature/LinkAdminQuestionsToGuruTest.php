...
class LinkAdminQuestionsToGuruTest extends TestCase
{
    ...
    public function test_command_backfills_teacher_guru_mapel_id_from_legacy_creator(): void
    {
        ...
        // Soal legacy: created_by_user_id sudah terisi, teacher_guru_mapel_id kosong.
        $question = $this->adminQuestion($subject, [$classA->id]);
        $question->update(['created_by_user_id' => $guru->user_id]);

        $this->artisan('exam:link-admin-questions')->assertSuccessful();

        $this->assertSame($guru->id, $question->fresh()->teacher_guru_mapel_id);
    }
