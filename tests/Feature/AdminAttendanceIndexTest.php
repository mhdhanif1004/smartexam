...
class AdminAttendanceIndexTest extends TestCase
{
    ...
    public function test_admin_attendance_index_shows_period_room_and_participant_summary(): void
    {
        ...
        $this->actingAs($admin)
            ...
            ->assertSee('Check-in:')
            ->assertSee('Hadir <strong>1</strong>', false)
            ->assertSee('Tidak Hadir <strong>1</strong>', false);
    }

    public function test_admin_attendance_index_counts_student_once_per_room_regardless_of_subjects(): void
    {
        $admin = User::factory()->admin()->create();
        $period = $this->period('Sesi 1');
